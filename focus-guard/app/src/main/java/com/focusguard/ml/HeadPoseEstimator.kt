...
class HeadPoseEstimator(private val context: Context) {
    ...
    suspend fun estimate(faceCrop: FaceCrop): PoseResult = withContext(Dispatchers.Default) {
        ...
        val landmarkInputBitmap = Bitmap.createScaledBitmap(faceBitmap, 192, 192, true)
        
        val pixels = IntArray(192 * 192)
        landmarkInputBitmap.getPixels(pixels, 0, 192, 0, 0, 192, 192)
        ...
        // --- Step 2: Eye Region Cropping & Preprocessing ---
        val width = maxX - minX
        ...
        val eyeCrop = cropBitmap(faceBitmap, eyeRect)
        if (eyeCrop.width < 10 || eyeCrop.height < 10) {
            ...
            return@withContext PoseResult(0f, 0f, 0f)
        }
        
        val eyeGazeInputFloats = prepareGazeInput(eyeCrop) // 96x160 grayscale
        ...
        PoseResult(yaw = yawDeg, pitch = pitchDeg, roll = 0f)
    }
